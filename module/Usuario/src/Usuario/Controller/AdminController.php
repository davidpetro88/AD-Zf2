...
use Usuario\Form\UserForm;
use Usuario\Form\UserFilter;
...
class AdminController extends AbstractActionController
{
    ...
    public function updateAction()
    {
        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                if (empty($data['usr_registration_date']))
                    $data['usr_registration_date'] = '2013-07-19 12:00:00';
                $this->getUsersTable()->update($data, array(
                    'usr_id' => $id
                ));
                return $this->redirect()->toRoute('auth-doctrine/default', array(
                    'controller' => 'admin',
            ...
            $form->setData($this->getUsersTable()
                ->select(array(
                'usr_id' => $id
            ))
                ->current());
    ...
    public function deleteAction()
    {
        $id = $this->params()->fromRoute('id');
        if ($id) {
            $this->getUsersTable()->delete(array(
                'usr_id' => $id
            ));
        }
